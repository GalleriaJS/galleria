...
Instance methods
****************
Instance methods are exposed in the galleria instance. You can access them via
the extend option or by grabbing the galleria instance using the static
:ref:`galleria-get` method. They are also exposed in the theme init method.
...
A proxy function that brings the Galleria scope to any callback. Using this
proxy, the ``this`` keyword stays as a reference to the current Galleria scope
during jQuery callbacks (or any other function). The second argument specifies
another scope (optional). Example::
