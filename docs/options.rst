...
**Note:**  You can define your own options and set defaults for each theme using the `Theme builder API <#>`_ .
...
* attribute (if exists and links to an image)
*  **thumb:**  
* the
* .. code:: text
...
default: **'img'** 
+++++++++++++++++++
The selector Galleria should look for in the HTML source. Defaults to.. code:: text

 'img'

and there is rarely any reason to change this.
...
*  **'auto'**  
* means that it will try to detect dataType automatially.
*  **'json'**  
* will force Galleria to treat the
* .. code:: text

 source


* parameter as a JSON data String or Array
*  **'html'**  
* will force Galleria to treat the
* .. code:: text
...
* parameter as a selector in the DOM where the image data is found.
...
default: **undefined** 
+++++++++++++++++++++++
This function is used to extend the init function of the theme. Use this to quickly add minor customizations to the theme. The first argument is the options object, and the scope is always the Galleria gallery, just like the theme's.. code:: text
...
*  **true**  
* means that all images will be scaled to fill the stage, centered and cropped.
*  **false**  
* will scale down so the entire image fits.
...
*  **2**  
* preloads the next 2 images in line
*  **'all'**  
* forces Galleria to start preloading all images. This may slow down client.
*  **0**  
* will not preload any images
...
*  **'auto'**  
* uses high quality if image scaling is moderate.
*  **false**  
* will not use high quality (better performance).
*  **true**  
* will force high quality renedring (can slow down performance)
...
default: **true** 
++++++++++++++++++
Sets the creation of thumbnails. If **false** , Galleria will create empty spans with the className.. code:: text
...
Please see the `Transitions documentation <#>`_ for more info.
...
*  **'fade'**  
* fade betweens images
*  **'flash'**  
* fades into background color between images
*  **'slide'**  
* slides the images using the Galleria easing depending on image position
*  **'fadeslide'**  
